...
export function FloatingSupport() {
  ...
  return <div className="fixed bottom-[calc(1.25rem+env(safe-area-inset-bottom))] right-5 z-[60] flex flex-col items-end gap-3">
    {open && <section onPointerDown={markInteraction} onFocus={markInteraction} className="fixed inset-x-3 bottom-24 max-h-[min(76dvh,38rem)] overflow-hidden rounded-xl border border-border bg-card shadow-2xl sm:static sm:w-[23rem]" aria-label="VisionSecure AI Assistant">
      <header className="flex items-center justify-between bg-primary px-4 py-3 text-primary-foreground"><div className="flex items-center gap-2"><Image src="/images/Visionsecuretech_logo.png" alt="VisionSecure" width={36} height={36} className="h-9 w-9 rounded-full bg-white object-contain p-1" /><div><span className="flex items-center gap-2 font-semibold"><Sparkles className="h-4 w-4" />VisionSecure AI Assistant</span><p className="mt-0.5 text-xs text-primary-foreground/80">Smart Security & IT Advisor</p></div></div><button type="button" onClick={() => { markInteraction(); sessionStorage.setItem("visionsecure-ai-intro-shown", "true"); setOpen(false) }} aria-label="Close VisionSecure AI Assistant" className="rounded p-1 transition hover:bg-white/15"><X className="h-4 w-4" /></button></header>
      ...
      <Link href="/contact" className="block border-t border-border px-4 py-3 text-center text-xs font-semibold text-primary hover:underline">Open contact form</Link>
    </section>}
    <div className="flex flex-col items-center gap-3"><a href={whatsappUrl} target="_blank" rel="noopener noreferrer" aria-label="Chat on WhatsApp" title="Chat on WhatsApp" className="flex h-14 w-14 items-center justify-center rounded-full bg-green-500 text-white shadow-lg shadow-green-500/30 transition hover:scale-105 hover:bg-green-600 motion-reduce:transition-none"><FaWhatsapp className="h-7 w-7" /></a><button type="button" onClick={() => { markInteraction(); setOpen((value) => !value) }} aria-label={open ? "Close VisionSecure AI" : "Ask VisionSecure AI"} title="Ask VisionSecure AI" aria-expanded={open} className="flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/30 transition hover:scale-105 hover:bg-primary/90 motion-reduce:transition-none"><Bot className="h-6 w-6" /></button></div>
  </div>
}
