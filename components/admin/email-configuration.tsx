...
export function EmailConfigurationPanel() {
  ...
  if (loading) return <AdminLoading title="Loading email configuration" description="Checking saved settings and service health..."/>
  return <div className="mx-auto max-w-6xl space-y-7 pb-12 text-foreground">
    ...
    {health && !health.encryptionKeyConfigured && <p className="rounded-xl bg-warning/10 p-4 text-warning">This server is missing EMAIL_CONFIG_ENCRYPTION_KEY. Local .env.local settings are not automatically deployed. In your hosting environment settings, securely set the same existing 64-character key used by the environment that saved these credentials, then redeploy/restart. Do not generate a replacement key for an existing shared configuration or enter the key in this form. <button type="button" className="ml-2 min-h-11 rounded-lg border border-current px-3 font-semibold" disabled={!!busy} onClick={async()=>{setBusy("health");try{setHealth(await request("/health"))}catch{setError("Unable to check server setup. Please retry.")}finally{setBusy("")}}}>Recheck server setup</button></p>}
    {config?.lastTestErrorCategory && <p role="alert" className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive"><strong>Last email check: {config.lastTestErrorCategory.replaceAll("_", " ")}. </strong>{config.lastTestErrorCategory === "CONFIGURATION_ERROR" ? "Confirm that this deployment uses the same encryption key as the server that saved the credentials." : config.lastTestErrorCategory === "CONNECTION_FAILED" ? "After checking the encryption setup, verify the SMTP hostname and outbound port access from this hosting environment. Recheck using the selected SSL 465 or STARTTLS 587 preset." : config.lastTestErrorCategory === "AUTHENTICATION_FAILED" ? "Zoho rejected the sign-in. Enter a valid newly created app password, save and verify again." : "Review delivery logs and verify the saved configuration before activation."}</p>}
    ...
    <div className="sticky bottom-0 z-30 flex flex-nowrap gap-3 overflow-x-auto sm:flex-wrap rounded-2xl border border-border bg-background/95 p-4 shadow-xl backdrop-blur-md" aria-busy={!!busy}>
      ...
      <button className={buttonClass} disabled={!canManage || !!busy || !health?.encryptionKeyConfigured || dirty || !config?.smtpHost} onClick={() => void action("verify")}>Test Connection</button>
      <button className={buttonClass} disabled={!canManage || !!busy || !health?.encryptionKeyConfigured || dirty || !config?.isVerified || !["VERIFIED", "TEST_EMAIL_SENT", "ACTIVE", "DISABLED"].includes(status)} onClick={() => void action("test")}>Send Test Email</button>
      <button className={buttonClass} disabled={!canManage || !!busy || !health?.encryptionKeyConfigured || dirty || !config || !canActivate(config) || status === "ACTIVE"} onClick={() => void action("activate")}>Activate Notifications</button>
