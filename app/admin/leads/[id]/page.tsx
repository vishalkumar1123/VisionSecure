...
export default function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-10 text-foreground">
    ...
    <div className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
      ...
      <aside className="space-y-6"><section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-xl font-bold">Next action</h2><label className="mt-5 block text-sm text-muted-foreground">Priority<select value={lead.priority || "Medium"} disabled={saving} onChange={(event) => void updateLead({ priority: event.target.value }, "Priority updated")} className="mt-2 h-11 w-full rounded-lg border border-border bg-background px-3 text-foreground">{LEAD_PRIORITIES.map((priority) => <option key={priority}>{priority}</option>)}</select></label><label className="mt-4 block text-sm text-muted-foreground">Follow-up date<input type="datetime-local" value={lead.followUpDate ? new Date(new Date(lead.followUpDate).getTime() - new Date(lead.followUpDate).getTimezoneOffset() * 60000).toISOString().slice(0, 16) : ""} disabled={saving} onChange={(event) => void updateLead({ followUpDate: event.target.value || null }, event.target.value ? "Follow-up scheduled" : "Follow-up cleared")} className="mt-2 h-11 w-full rounded-lg border border-border bg-background px-3 text-foreground" /></label>{lead.followUpDate && <p className="mt-4 flex items-center gap-2 rounded-lg bg-warning/10 p-3 text-sm text-warning"><CalendarClock className="h-4 w-4" />{new Date(lead.followUpDate).toLocaleString("en-IN")}</p>}</section>
